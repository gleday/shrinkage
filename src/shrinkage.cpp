...
// [[Rcpp::export(.sgridge)]]
Rcpp::List sgridge(arma::colvec y, arma::mat X, arma::colvec g, arma::colvec priors, double a = 0.00001, double b = 0.00001, double c = 0.00001, const int mcmc = 1000, const int  burnin = 1000, const int thin = 10, bool verbose = true, const int step = 1000){
  ...
  const double cStar = 0.5*(n+p);
  double tauminus2 = 1;
  arma::colvec lambdaminus2 = ones(K);
  double sigmaminus2 = 1;
  double gamma2 = 1;
  const int nruns = mcmc + burnin;
  arma::colvec wk = ones(K)*1/K;
  arma::colvec d = zeros(p);
  for(int u = 0; u < K; u++){
    d(find(g == gr(u))).fill(lambdaminus2(u));
  }
  arma::mat D = diagmat(d);
  ...
  arma::mat betasamp = zeros(mcmc, p);
  arma::colvec tau2samp = zeros(mcmc);
  arma::mat lambda2samp = zeros(mcmc, K);
  arma::colvec sigma2samp = zeros(mcmc);
  arma::mat betavar, cholvar;
  arma::colvec betamean, resid;
  arma::colvec beta(p);
  ...
  for(int i = 0; i < nruns; i++){
    for(int j = 0; j < th; j++){
      ...
      // Sample from P(\tau^2 | ...)
      if(priors(0) == 1){ // if \tau^2 ~ invGamma(a, b)
        tauminus2 = R::rgamma(a + 0.5*p, 1/(b + 0.5*sigmaminus2*btDb));
      }
      if(priors(0) == 2){ // if \tau_k^2 ~ BetaPrime(a, b)
        tauminus2 = 1/rgig(a - 0.5*p, sigmaminus2*btDb, 2*gamma2);
        gamma2 = R::rgamma(a+b, 1/(1/tauminus2 + 1));
      }
      if(priors(0) == 3){ // if \tau_k^2 ~ invGaussian(a, b)
        tauminus2 = 1/rgig(-0.5 - 0.5*p, b + sigmaminus2*btDb, b/(a*a));
      }
      if(priors(0) == 4){ // if \tau_k^2 ~ Gamma(a, b)
        tauminus2 = 1/rgig(a - 0.5*p, sigmaminus2*btDb, 2*b);
      }
      
      // Sample from P(\lambda_k^2 | ...)
      if(priors(1) == 1){ // if \lambda_k^2 ~ Gamma(c, c)
        for(int u = 0; u < K; u++){
          indk = find(g == gr(u));
          bktbk = sum(square(beta(indk)));
          lambdaminus2(u) = 1/rgig(c - 0.5*pk(u), sigmaminus2*tauminus2*bktbk, 2*c);
        }
      }
      if(priors(1) == 2){ // if \lambda_k^2 ~ invGaussian(1, c)
        for(int u = 0; u < K; u++){
          indk = find(g == gr(u));
          bktbk = sum(square(beta(indk)));
          lambdaminus2(u) = 1/rgig(-0.5 - 0.5*pk(u), c + sigmaminus2*tauminus2*bktbk, c);
        }
      }
    ...
    if(i >= burnin){
      ...
      betasamp.row(k) += beta.t();
      tau2samp(k) += 1/tauminus2;
      lambda2samp.row(k) += 1/(lambdaminus2.t());
      sigma2samp(k) += 1/sigmaminus2;
      k++;
      ...
      if(k%step==0){
        // estimate c
        double eloglambda2k = mean(mean(log(lambda2samp.rows(0, k-1)), 0));
        Rcpp::Rcout << "eloglambda2k = " << eloglambda2k << std::endl;
        double elambda2k = mean(mean(lambda2samp.rows(0, k-1), 0));
        Rcpp::Rcout << "elambda2k = " << elambda2k << std::endl;
        const auto obj = [elambda2k, eloglambda2k](double c) { return log(c) - R::digamma(c) + eloglambda2k - elambda2k + 1; };
        boost::uintmax_t it = 1000;
        eps_tolerance<double> tol(20);
        const auto result = bracket_and_solve_root(obj, c, 2.0, true, tol, it);
        auto val1 = 0.0, val2 = 0.0;
  ...
  Rcpp::List out = Rcpp::List::create(
    Rcpp::Named("betas") = betasamp,
    Rcpp::Named("tau2s") = tau2samp,
    Rcpp::Named("lambda2s") = lambda2samp,
    Rcpp::Named("sigma2s") = sigma2samp,
    Rcpp::Named("a") = a,
    Rcpp::Named("b") = b,
    Rcpp::Named("c") = c
